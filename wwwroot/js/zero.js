(function (zWin) {
	...
	if (zWin.useTouchScreenClick === undefined) {
		zWin.useTouchScreenClick = true
	}
	if (!zWin.isWin) {
	...
	if (!Array.prototype.clearItems) {
		Array.prototype.clearItems = function (k) {
			for (var g = 0; g < this.length;) {
				k ? this[g] === k ? this.splice(g, 1) : g++ : this[g] ? g++ : this.splice(g, 1)
			}
			return this
		}
	}
	...
	var animationFrames = 12,
		...
		getDebugTextarea = function () {
			...
			if (!div.length) {
				div = $(document.createElement('div')).attribute('id', zeroDebugBarDivId).html('<div><p><a>debug</a><b onclick="zero(this).parent(3).remove()">x</b></p><textarea></textarea></div>').appendTo(body).find('textarea')[0]
			} else {
		...
		getSize = function (ele, isExcludeBorderWidth) {
			if (isEle(ele)) {
				...
				return {
					width: Math.max(ele.documentElement.clientWidth, ele.documentElement.scrollWidth),
					height: Math.max(ele.documentElement.clientHeight, ele.documentElement.scrollHeight)
				}
			} else if (isWin(ele)) {
		...
		doAanimation = function (func, para) {
			...
			var reval;
			if (isAnimationFrame) {
				reval = requestAnimationFrame(function () {
					func(para)
				})
			} else {
	...
	pointHelp = {
		...
		p2pChange: function (ele) {
			var func = function (ele) {
				...
				var _targetPoint, _change, _end, _step = 1,
					_begin = function () {
						if (!isFunc(_change)) {
							return
						}
						...
						var pInfo = pointHelp.p2pRelation(startPoint.x, startPoint.y, _targetPoint.x, _targetPoint.y),
							...
							len = ps.length,
							num = 1,
							interval, frame = len < animationFrames ? len : (len % animationFrames === 0 ? len / animationFrames : parseInt(len / animationFrames, 10) + 1),
							run = function () {
								if (index < len) {
									try {
				...
				var _targetSize, _change, _end, _step = 1,
					_begin = function () {
						...
						var sizeInfo = sizeHelp.getChangeInfo(startSize, _targetSize),
							index = 0,
							len = sizeInfo.size.length,
							num = 1,
							interva, frame = len < animationFrames ? 1 : (len % animationFrames === 0 ? len / animationFrames : parseInt(len / animationFrames, 10) + 1),
							run = function () {
								if (index < len) {
									try {
										var rate = Math.round((index + 1) / len * 100);
										_change.apply(ele, [{
											width: sizeInfo.size[index].width,
											height: sizeInfo.size[index].height
										},
											rate, _step, sizeInfo.distance])
									} catch (e) {
										log(e.message)
									}
									interval = doAanimation(run)
		...
		change: function (ele) {
			var func = function (ele) {
				...
				var _targetOpacity, _change, _end, _step = 1,
					_begin = function () {
						if (!isFunc(_change)) {
							return
						}
						...
						var opacityInfo = opacityHelp.getChangeInfo(startOpacity, _targetOpacity),
							index = 0,
							len = opacityInfo.opacity.length,
							num = 1,
							interval, frame = len < animationFrames ? 1 : (len % animationFrames === 0 ? len / animationFrames : parseInt(len / animationFrames, 10) + 1),
							run = function () {
								if (index < len) {
									try {
										var rate = Math.round((index + 1) / len * 100);
		...
		tap: function (ele, callback) {
			...
			var fun = function (ele, callback) {
				var _this = this,
					sleep = 300,
					distance = 40,
					beginTime, beginTouches, start = function (e) {
						beginTime = new Date();
						...
						if (e.touches.length < 2 && (e.target === ele || ele.contains(e.target))) {
							...
							if (distance2 < distance && sleep > duration && isFunc(callback) && e.target) {
		...
		doubleTap: function (ele, callback) {
			...
			var fun = function (ele, callback) {
				var _this = this,
					sleep = 300,
					distance = 40,
					beginTime, beginTouches, touchCount = 0,
					start = function (e) {
						...
						if (touchCount === 2 && e.touches.length < 2 && (e.target === ele || ele.contains(e.target))) {
							...
							var distance2 = e.touches.length < 1 ? 0 : Math.pow((e.targetTouches[0].clientX * beginTouches[0].clientX + e.targetTouches[0].clientY * beginTouches[0].clientY), 0.5),
								duration = (new Date()).getTime() - beginTime.getTime();
							if (distance2 < distance && sleep > duration && 'function' === typeof (callback)) {
								callback.apply(ele, arguments)
							}
						}
					};
		...
		if (/^(\w+)(\s*)(=|\!|>|<)(\s*)(.*)$/.exec(expression)) {
			reval = {
				property: RegExp.$1.toString(),
				operator: RegExp.$3.toString(),
				value: RegExp.$5.toString()
			}
			...
			if (!isFunc(evFunc)) {
				for (var i = 0; i < this.length; i++) {
					if (!flag) {
						...
						eventExecute(this[i], 'touchstart', {
							targetTouches: [{
								clientX: 0,
								clientY: 0
							}],
	...
	$.ready = function (func) {
		...
		if (undefined != doc.readyState) {
			var g = setInterval(function () {
				/^(loaded|complete)$/.test(doc.readyState) && (clearInterval(g), end())
			}, 0);
			return
		}
	};
		...
		return new function (ele) {
			...
			var _this = this,
				...
				mouseUp = function (e) {
					e || (e = win.event);
					var point = getOffsetPoint(e.clientX, e.clientY);
					ele.releaseCapture ? (ele.onmousemove = ele.onmouseup = null, ele.releaseCapture()) : ($(doc).removeEvent("mousemove", mouseMove).removeEvent("mouseup", mouseUp));
	...
	$.ajax = function (url) {
		...
		return new function (url) {
			var _this = this,
				headData, reqMethod = 'GET',
				postData, queryData, errorCallback, sendCallback, beginTime, delay = 500,
				getRequest = function () {
					var req;
					if (zWin.XMLHttpRequest) {
							...
							try {
								...
								if (4 === req.readyState) {
									if (200 === req.status) {
										...
										doError({
											message: 'status=' + req.status + '&statusText=' + req.statusText
										})
									}
								}
							} catch (e) {
		...
		return new function (ele) {
			var _this = this,
				...
				inputTips = function (input, isError, msg) {
					...
					try {
						while (n != bod && !$(n).hasClass('zero_form_input')) {
							n = n.parentNode
						}
						if (n == bod || n.parentNode.nodeName != 'DD') {
							return
						}
						if (isError) {
							...
							$(n.parentNode).removeClass('zero_tips')
						}
					} catch (e) {
				...
				init = function () {
					...
					var getName = function (node, index) {
						...
						focusFunc = function (input) {
							...
							if ($(input).attribute('data-zfifeb')) {
								return
							}
			...
			this.submit = function () {
				if (this.isForceAjax && !isFunc(this.onResponse)) {
					return
				}
				if (!isFunc(this.onCheck)) {
					return
				}
				var myIframe = null;
				var myForm = null;
				...
				try {
					...
					this.onCheck(data);
					var errorCount = 0,
						tipsFunc = isFunc(this.onCustomTips) ? this.onCustomTips : doTips;
					...
					if (hasFiles()) {
						...
						myIframe = $(document.createElement('iframe')).cssText('display:none;').attribute('id', id).attribute('name', id).appendTo(document.body);
						myForm = $(document.createElement('form')).cssText('display:none;').attribute('id', id + 'form_').attribute('action', this.action).attribute('method', 'post').attribute('target', id).appendTo(document.body);
						...
						var ajax = $.ajax(this.action).error(function (e) {
							return e.message
						}).method(this.method);
	...
	$.dialog = new function () {
		var thisFlag = 'p' + (new Date().getTime()),
			...
			drag = function (dialogObj) {
				if (!dialogObj.dialogNode) {
					return
				}
				var obj = dialogObj,
					p = $(obj.dialogNode).find('div', 1)[1],
					o = $(p, "div").find('div', 1)[2];
				o.style.cursor = "move";
				var showDiv = $(o).parent(2)[0];
				dialogObj.dragObj = $.drag(o).begin(function (node, point, para) {
					this.para({
						maxX: para.maxWidth - showDiv.offsetWidth,
						maxY: para.maxHeight - showDiv.offsetHeight
					})
			...
			getRootNode = function (appendBody, id) {
				id = id ? id : 'dialogInit';
				var t = dialogObj.doc.getElementById(id);
				if (!t) {
					var t = document.createElement('div');
					t.style.cssText = 'position:absolute;z-index:9999;left:0;top:0;';
					t.id = id;
					$(t).appendTo(dialogObj.bod)
				}
				return t
			},
			docKeydownFun = function (e) {
				var k = 0;
				e = e || dialogObj.win.event;
			...
			defaultConfig = {
				title: lan.dialog,
				content: '<center>dialog message</center>',
				btnConfig: [{
					text: lan.ok,
					func: function () {
						this.close()
		...
		this.show = function (config) {
			...
			return new function () {
				...
				if (this.doc.documentElement) maskDiv.cssText('height:' + size.height + 'px');
				...
				var endFunc = function () {
					...
					getMousePos = function (e) {
						...
						try {
							...
							if (n.nodeName === 'IMG' || n.nodeName === 'IFRAME') {
								return p1
							}
							if (e.clientX === undefined || (e.clientX === 0 && e.clientY === 0)) {
								return p1
							}
							var nDoc = n.ownerDocument,
								nWin = nDoc.defaultView || nDoc.parentWindow || zWin;
							p2.win = nWin;
							return p2
						} catch (e) {
							return p1
				...
				var toPoint = {
					x: cW / 2 - boxW / 2 + scrollL,
					y: cH / 2 - boxH / 2 + scrollT
				},
					toSize = {
						width: boxW,
						height: boxH
		...
		this.alert = function (title, content, onClosed, width) {
			return this.show({
				title: isStr(title) ? title : 'alert',
		...
		this.confirm = function (title, content, onResult, width) {
			return this.show({
				title: isStr(title) ? title : 'confirm',
				content: '<div style="padding:5px;">' + content + '</div>',
				btnConfig: [{
					text: lan.no,
					func: function () {
		...
		this.prompt = function (title, onResult, value, mode, width) {
			...
			return this.show({
				title: isStr(title) ? title : 'prompt',
				content: '<div style="padding:5px;">' + s + '</div>',
				btnConfig: [{
					text: lan.cancel,
					func: function () {
						this.close()
					...
					text: lan.sure,
					func: function () {
						onResult.apply(this, [this.form().getData().prompt_input])
					},
					isPrimary: true
				}],
				width: width,
				onShowed: function () {
					var o = $(this.form().inputs.prompt_input).attribute('placeholder', 'Enter content here').value(value)[0];
					o.focus();
					if (value.length < 1) {
						return
					}
		...
		this.tips = function (content, interval, width) {
			interval = interval ? interval : 700;
			return this.show({
				content: '<div style="padding:5px;">' + content + '</div>',
				width: width,
				showHeader: false,
				onShowed: function () {
					...
					setTimeout(function () {
						o.close()
					}, interval < 700 ? 700 : interval)
				}
			})
		};
		this.contentOnly = function (content, onShowed, width) {
			return this.show({
				content: content,
				onShowed: onShowed,
				width: width,
		...
		this.loadIframe = function (title, url, width, height, onLoad, onDataCallback, onClosed) {
			...
			return this.show({
				title: title,
				content: '<div class="zero_dialog_iframe_box" style="width:' + (width) + 'px;height:' + (height - 37) + 'px;overflow:hidden;padding:0;margin:0;border:0;"></div>',
				width: width,
				showHeader: true,
				onClosed: onClosed,
				onShowed: function () {
		...
		return new function (title) {
			...
			var getDatetimeFormat = function (val, format) {
				...
				show = function () {
					...
					if (target.type !== 'text' && target.type !== 'datetime') {
						return
					}
					target.blur();
					doc = target.ownerDocument, win = doc.defaultView || doc.parentWindow || zWin;
					var root = getRoot(),
					...
					var showDiv = $(root).find('class=box'),
						...
						point = $(target).getAbsPoint(true),
						maxX = Math.max(winSize.width, docSize.width) - showDivSize.width,
						maxY = Math.max(winSize.height, docSize.height) - showDivSize.height;
					if (point.x > maxX) {
						point.x = maxX;
					}
					if (point.y > maxY) {
	...
	$.requireJs = function (url, onload) {
		$.ready(function () {
			require.js(url, onload)
		})
	};
	$.requireCss = function (url, onload) {
		$.ready(function () {
			require.css(url, onload)
		})
	};
	$.codeHighlight = {
		_: function (name, color, s) {
			return '<' + name + ' style="color:#' + color + '">' + s + '</' + name + '>'
		},
		html: function (s) {
			s = s.htmlEncode();
			var _this = this,
	...
	$.decompressionListData = function (d) {
		if (!isArray(d) || d.length < 3 || isNaN(d[0])) {
			return d
		}
